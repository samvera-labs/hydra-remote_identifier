...
describe Hydra::RemoteIdentifier do
  context "integration spec" do
    ...
    describe '.register API' do
      ...
      before(:each) do
        Hydra::RemoteIdentifier.configure do |config|
          config.remote_service(:doi, doi_options)
        end

        Hydra::RemoteIdentifier.register(:doi, target_class) do |map|
          map.target :url
          map.creator :creator
          map.title :title
          map.publisher :publisher
